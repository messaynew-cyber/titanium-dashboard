...
class TitaniumService:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.latest_signal = {
                "sentiment": "WAITING", "strength": 0, "reason": "System Initializing...", 
    ...
    def generate_signal_now(self):
        """Forces an immediate market scan and returns the signal."""
    ...
    def load_history(self):
        ...
        for i in range(50, 0, -1):
            ...
            ghost_data.append({
                "timestamp": t, "value": base + random.uniform(-5, 5), "price": price + random.uniform(-0.1, 0.1)
            })
        return ghost_data
    ...
    def _run_loop(self):
        ...
        while self.running:
            try:
                ...
                # AUTOPILOT LOGIC (Calling the shared signal generator)
                # We use the same logic as manual scan, but then execute
                sig_data = self.generate_signal_now() # Re-use logic
                
                # EXECUTE IF VALID
                if "error" not in sig_data:
                    signal = {"position_pct": 0.40 if sig_data['sentiment'] == 'BULLISH' else -0.40}
                    
                    target_qty = int((real_equity * signal['position_pct']) / price)
