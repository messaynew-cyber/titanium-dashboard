      ...
      {tab === 'logs' && (
        <div className="glass-card h-[800px] flex flex-col animate-in fade-in">
           ...
           <div className="flex-1 overflow-y-auto p-6 font-mono text-xs space-y-2">
             {logData.map((l:any, i:number) => <div key={i} className="border-b border-white/5 pb-1 opacity-70 hover:opacity-100"><span className="text-accent mr-2">[{l.level}]</span>{l.message}</div>)}
           </div>
        </div>
      )}
    </div>
